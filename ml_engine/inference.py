...
def detect_anomaly(payload: str, request_rate: int) -> bool:
    ...
    if artifact and 'model' in artifact and 'scaler' in artifact:
        try:
            ...
            X = np.array([[payload_len, float(request_rate), float(suspicious_count)]])
            X_scaled = scaler.transform(X)
            prediction = model.predict(X_scaled)[0]
            # IsolationForest returns -1 for anomaly, 1 for normal
            is_anomaly = bool(prediction == -1 or suspicious_count > 0 or request_rate > 100 or payload_len > 5000)
            return is_anomaly
        except Exception as e:
            print(f"⚠️ Warning: Model inference error ({e}). Falling back to heuristic rules.")

    # Fallback logic if model.pkl is missing or error occurs
    heuristic_anomaly = (
        request_rate > 100 or
        payload_len > 5000 or
        suspicious_count > 0
    )
    return bool(heuristic_anomaly)
